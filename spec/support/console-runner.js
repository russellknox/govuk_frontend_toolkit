...
(function (jasmine, console) {
  if (!jasmine) {
    throw new Error("jasmine library isn't loaded!")
  }
  ...
  ANSI.colorize_text = function (text, color) {
    var colorCode = this.color_map[color]
    return '\x1B[' + colorCode + 'm' + text + '\x1B[0m'
  }

  var ConsoleReporter = function () {
    if (!console || !console.log) { throw new Error("console isn't present!") }
  ...
  proto.reportRunnerResults = function (runner) {
    ...
    var specStr = this.executed_specs + (this.executed_specs === 1 ? ' spec, ' : ' specs, ')
    ...
    this.log('Finished')
    this.log('-----------------')
    this.log(specStr + failStr + (dur / 1000) + 's.', color)
  ...
  proto.log = function (str, color) {
    var text = (typeof color !== 'undefined') ? ANSI.colorize_text(str, color) : str
    console.log(text)
  }

  jasmine.ConsoleReporter = ConsoleReporter
})(window.jasmine, console)
